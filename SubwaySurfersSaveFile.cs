...
public class SubwaySurfersSaveFile
{
    ...
    public void ImportFile(string fileName = null)
    {
        ...
        Console.WriteLine($"File hash size: {hashSize}");

        // Get the hash
        byte[] hash = new byte[hashSize];
        reader.Read(hash, 0, hashSize);
        Console.WriteLine($"File hash: {BitConverter.ToString(hash).Replace("-", "").ToLower()}");
        ...
        for (int i = 0; i < numStrings; i++)
        {
            ...
            Entries[key] = value;
            Console.WriteLine($"Read string {i}: {key} -> {value}");
        }

        // Read the first chunk of numbers
        int numInts = ReadDword(reader);
        Console.WriteLine($"Number of integers (first set): {numInts}");
        for (int i = 0; i < numInts; i++)
        {
            string key = ReadString(reader);
            int value = ReadDword(reader);
            FirstNumberEntries[key] = value;
            Console.WriteLine($"Read number {i}: {key} -> {value}");
        }

        // Read the second chunk of numbers
        numInts = ReadDword(reader);
        Console.WriteLine($"Number of integers (second set): {numInts}");
        for (int i = 0; i < numInts; i++)
        {
            string key = ReadString(reader);
            int value = ReadDword(reader);
